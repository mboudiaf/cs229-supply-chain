...
#    price_elec = data.values
#    price_spot = price_elec[::-1,:]
    return data.values
...
def optim_main(design_conv, N_conv, design_batt, N_batt, design_vess, N_vess, Psi_solar, Psi_wind, c_H2_industry, c_H2_mobility, N_d, path_data):
    ...
    for i in range(N_t):
        ...
        Phi_vess_in[i] = s.NumVar(-s.infinity(), s.infinity(), 'Phi_vess_in_'+str(i))
    ...
    for i in range(N_t):
        ...
        for j in range(5):
            Phi_node[N_t*j+i] = s.NumVar(-s.infinity(), s.infinity(),'Phi_node_'+str(i)+str(j))
...
#     (6) On/Off policy for the electrolyzer fleet
    ...
    for i in range(N_t):
        ...
        objective.SetCoefficient(Psi_node[N_t*0+i],-Eps_spot[i]) # net purchase from the grid for the battery park
        objective.SetCoefficient(Psi_node[N_t*3+i],-Eps_spot[i]) # net purchase from the grid for the battery park

...
from pybrain.structure.modules import Table, Module, TanhLayer, LinearLayer, BiasUnit, ReluLayer
...
class ActionValueNetwork(Module, ActionValueInterface):
    ...
    def __init__(self, dimState, numActions, name=None,id2action=None):
        ...
        self.network = buildNetwork(dimState + numActions,dimState + numActions,int((dimState + numActions)/2),1,hiddenclass=ReluLayer)
    ...
    def getMaxAction(self, state):
        ...
        if nb_max_states>1: # if several actions give the same Q value
            max_indices = [i for i in range(len(values)) if values[i]==maxvalue]
            chosen_index = random_choice(max_indices)
            print("Number of max states: ",nb_max_states, "chosen index: ",chosen_index) 
        return chosen_index
    ...
    def getActionValues_test(self, state):
        """ Run forward activation for each of the actions and returns all values. """
        values = array([self.network.activate(r_[state, action]) for action in self.id2action.values()])
        return values

    def getValue(self, state, action):
        return self.network.activate(r_[state, one_to_n(action, self.numActions)])
    
    def getValue_test(self, state, action):
        return self.network.activate(r_[state, action])